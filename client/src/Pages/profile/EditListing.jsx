...
//     // Sanitize and ensure numeric fields have proper values or fallback to 0
//     const sanitizedFormData = {
//       tipropertyNamele: inputs.propertyName || "",
//       price: parseInt(inputs.price, 10) || 0,
//       address: inputs.address || "",
//       city: inputs.city || "",
//       bedroom: parseInt(inputs.bedroom, 10) || 0,
...
function EditListing() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error("❌ " + (err.response?.data?.error || "Error updating listing."), {
        position: "bottom-right",
        autoClose: 5000,
        hideProgressBar: false,
        closeOnClick: true,
      ...
      setError(err.response?.data?.message || "Error updating the post. Please try again.");
    }
  };
        ...
        <div className="row">
          <div className="formContainer col-lg-8">
            ...
            <form onSubmit={handleSubmit} className="form">
              ...
              <div className="item description">
                <label htmlFor="desc">Description</label>
                <ReactQuill theme="snow" value={value} onChange={setValue} placeholder="Edit your description here..." />
              </div>
              <div className="submitSection">
                <button className="btn btn-outline-primary">Update Listing</button>
                {error && <span className="errorMessage">{error}</span>}
              </div>
            </form>
          </div>
          ...
          <div className="sideContainer col-lg-4 text-center">
            ...
            <div className="imagesPreview">
              {images.length > 0 ? (
                images.map((image, index) => <img src={image} key={index} alt="Uploaded" />)
              ) : (
                <div className="defaultIcon"><p>No images uploaded!</p></div>
              )}
            </div>
            {hasPreviousImages && <button onClick={handleResetImages} className="btn btn-outline-danger">Reset Images</button>}
            <UploadWidget
              uwConfig={{
                cloudName: import.meta.env.VITE_CLOUDINARY_CLOUD_NAME,
                uploadPreset: import.meta.env.VITE_CLOUDNARY_UPLOAD_PRESET,
